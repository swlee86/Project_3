...
$(function(){
	$('#susin').click(function(){
			$('#url').attr('href','taskInform.do?tab_char=1');
	});
	$('#songin').click(function(){
		$('#url').attr('href','taskInform.do?tab_char=2');
});
});
...
<div class="normalheader transition animated fadeIn">
	<div class="hpanel">
		<div class="panel-body">
			...
			<h2 class="font-light m-b-xs">업무 보고  <span><a id="url" href="taskInform.do?tab_char=1"><i class="fa fa-repeat"></i></a></span></h2>
				...
				<!-- 수신 -->
					<div id="tab-1" class="tab-pane <c:if test="${tab_char==1}">active</c:if>">
										...
										<tr>
											...
											<td>
												...
												<!-- <button class="btn btn-sm"
														...
														onclick="window.location.href='taskInform.do'">
						...
						<div class="panel-footer" style="text-align: center">
							<div class="btn-group">
								<c:if test="${pg_rec>1}">
									<a  class="btn btn-default" href="taskInform.do?pg_rec=${pg_rec-1}&f_rec=${field_rec}&q_rec=${query_rec}&tab_char=1">
								...
								<c:forEach var="i" begin="1" end="${pagecount_rec}">
									<c:choose>
										...
										<c:otherwise>
											<a class="btn btn-default" href="taskInform.do?pg_rec=${i}&f_rec=${field_rec}&q_rec=${query_rec}&tab_char=1">
												${i}
											</a>
										</c:otherwise>
									</c:choose>
					...
					<div id="tab-2" class="tab-pane <c:if test="${tab_char==2}">active</c:if>">
									...
									<table style="margin-top: 10px; margin-bottom: 10px;" width="100%">
										<tr>
											...
											<select class="form-control input-sm" 
												...
												<option value="task_name">업무 명</option>
												<option value="deadline">업무 기한</option>
												<option value="emp_no">승인자</option>
												<option value="send_date">작성일</option>
											</select>
											</th>
											<!-- 검색 input 창 -->
									...
									<tbody id="secondTable">
										<c:forEach var="list2" items="${list2}">
											<tr>
												...
												<td style="padding-top: 12px;">
													<a href="taskInform_Detail.do?task_no=${list2.task_no}">
														${list2.task_name}
													</a>
												</td>
						...
						<div class="panel-footer" style="text-align: center">
							<div class="btn-group">
								<c:if test="${pg_song>1}">
									<a  class="btn btn-default" href="taskInform.do?pg_song=${pg_song-1}&f_song=${field_song}&q_song=${query_song}&tab_char=2">
										&nbsp;<i class="fa fa-chevron-left"></i>
									</a>
								</c:if>
								...
								<c:forEach var="i" begin="1" end="${pagecount_song}">
									<c:choose>
										...
										<c:otherwise>
											<a class="btn btn-default" href="taskInform.do?pg_song=${i}&f_song=${field_song}&q_song=${query_song}&tab_char=2">
												${i}
											</a>
										</c:otherwise>
									</c:choose>
