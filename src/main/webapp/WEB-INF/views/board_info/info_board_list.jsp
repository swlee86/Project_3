				...
				<div class="panel-body">
					<div class="row">
						...
						<div class="text-right">
							<form id="searchForm" action="info_board_list.do"
									...
									<div class="form-group">
										<div class="input-group">
											<input type="text" class="form-control input-sm" name="q"
												id="title" /> <span class="input-group-btn">
												<button id="searchBtn"
													class="btn btn-sm btn-default input-sm" type="submit"
													style="color: #f05050">
							...
							<tbody>
								<c:forEach var="list" items="${companyList}">
									<tr>
										...
										<td><a
											href="detailinfo_board_list.do?no=${list.no}&currentpage=${cpage}&pagesize=${psize}">
												${list.title} <c:if test="${list.file_name != null}">
						...
						<div class="row" style="text-align: right; margin-right: 5px;">
							<se:authorize access="hasRole('ROLE_COMPANY')">
								<button type="button" class="btn w-xs btn-success"
									onclick="location.href='info_board_write.do'">글 등록</button>
							</se:authorize>
						</div>
					</div>
					...
					<div class="btn-group">
						<c:if test="${cpage > 1}">
							<button type="button" class="btn btn-default"
								onclick="location.href='info_board_list.do?currentpage=${cpage-1}&pagesize=${psize}'">
							...
							<c:choose>
								...
								<c:otherwise>
									<button class="btn btn-default"
										onclick="location.href='info_board_list.do?currentpage=${i}&pagesize=${psize}'">${i}</button>
								</c:otherwise>
							</c:choose>
						</c:forEach>
