									...
									<tbody>
										<tr>
												...
												<div class="form-group">
													<div class="input-group">
														<input type="text" class="form-control input-sm"
															...
															value="${contact.name}" /> <span class="input-group-btn">
											...
											<td>
												<div class="form-inline">
													<input type="text" class="form-control input-sm"
														id="addenrollDate" name="birth" value="${contact.birth}">
												</div>
											</td>
										</tr>
										<tr>
											...
											<td><input type="text" class="form-control input-sm"
												placeholder="회사명을 입력하세요." id="attach" name="attach"
												value="${contact.attach}"></td>
											<th
												style="background-color: #f5f5f5; text-align: right; padding-right: 10px; width: 10%">메일</th>
											<td><input type="text" class="form-control input-sm"
												placeholder="메일을 입력하세요." id="mail" name="mail"
												value="${contact.mail}"></td>
										</tr>
										<tr>
											<th
												style="background-color: #f5f5f5; text-align: right; padding-right: 10px; width: 10%">연락처1</th>
											<td><input type="text" class="form-control input-sm"
												placeholder="연락처를 입력하세요." id="tel1" name="tel1"
												value="${contact.tel1}"></td>
											<th
												style="background-color: #f5f5f5; text-align: right; padding-right: 10px; width: 10%">연락처2</th>
											<td><input type="text" class="form-control input-sm"
												placeholder="연락처를 입력하세요." id="tel2" name="tel2"
												value="${contact.tel2}"></td>
										</tr>
										<tr>
											<th
										...
										<tr>
											...
											<td colspan="3"><select class="form-control input-sm"
												style="height: 80px" size="3" name="group_no">
													<option value="1"
														<c:if test="${1 == contact.group_no}">selected</c:if>>선택안함</option>
													<c:forEach items="${grouplist}" var="g">
														...
														<c:choose>
															...
															<c:otherwise>
																<option value="${g.group_no}"
																	<c:if test="${g.group_no == contact.group_no}">selected</c:if>>${g.group_name}</option>
															</c:otherwise>
														</c:choose>
													</c:forEach>
											</select></td>
										...
										<tr>
											...
											<td colspan="3"><textarea class="form-control"
													name="memo">${contact.memo}</textarea></td>
										</tr>
									</tbody>
								</table>
