		...
		<div class="panel-body">
			...
			<div id="hbreadcrumb" class="pull-right m-t-lg">
				<ol class="hbreadcrumb breadcrumb">
					...
					<li class="active"><span>메모</span></li>
				</ol>
			</div>
			<h2 class="font-light m-b-xs">개인 메모</h2>
		...
		<div class="col-md-3">
			<div class="hpanel panel-group">
						...
						<c:otherwise>
							<c:forEach var="list" items="${list}">
								<div class="panel-body note-link"
									...
									<a href="#" class="memo_detail" id="${list.memo_no}"> <small
										class="pull-right text-muted">${list.memo_date}</small>
										<h5>${list.title}</h5>
				...
				<div class="panel-body" style="text-align: center">
					<c:choose>
						<c:when test="${empty pg}">
						</c:when>
						<c:otherwise>
							<div class="btn-group">
								<c:if test="${pg>1}">
									<a class="btn btn-default"
										href="private_memo.do?pg=${pg-1}&q=${q}&ps=${ps}"> &nbsp;<i
								...
								<c:forEach var="i" begin="1" end="${pagecount}">
									<c:choose>
										...
										<c:otherwise>
											<a class="btn btn-default"
												href="private_memo.do?pg=${i}&q=${q}&ps=${ps}"> ${i} </a>
										</c:otherwise>
									</c:choose>
								</c:forEach>
			...
			<div class="col-md-9">
				<div class="hpanel">
					<div class="panel-body" style="background: ${first_memo.color};"
						id="memo_detail_panel_body_color">
						<c:choose>
							<c:when test="${empty first_memo}">
								<div class="tab-content">
									<a href="#" id="first_memo_plus"><span
										class="glyphicon glyphicon-plus"></span></a>
									<div class="text-center">메모가 없습니다.</div>
								</div>
							</c:when>
							<c:otherwise>
								<div class="tab-content">
									...
									<div class="pull-right">
										<a href="removememo.do?memo_no=${first_memo.memo_no}"
											id="first_memo_delete"><b><span
												class="glyphicon glyphicon-trash"></span></b></a>
									</div>
									<input type="hidden" id="first_update_memo_no"
										value="${memo_no_chk}">
									<div id="note1" class="tab-pane active">
										<div class="pull-right text-muted m-l-lg"
											id="memo_detail_date">${first_memo.memo_date}</div>
										<h3>
											<input type="text" id="first_memo_title"
												value="${first_memo.title}"
												style="border:0px;background-color:${first_memo.color}">
										</h3>
										<hr />
										<div class="note-content"
											style="background: ${first_memo.color};"
											id="memo_detail_background">
											<textarea class="form-control"
												style="background: ${first_memo.color};"
												id="first_memo_content">${first_memo.memo_content}</textarea>
										</div>
									</div>
								</div>
							</c:otherwise>
